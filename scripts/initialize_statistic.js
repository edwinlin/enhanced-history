...
function setDefaults() {
  ...
  // Set number of days history timer has been used
...
function checkDate() {
  ...
  if (saved_day !== todayStr) {
    ...
    for (var domain in domains) {
      var domain_data = JSON.parse(localStorage[domain]);
      domain_data.yesterday = domain_data.today;
      domain_data.today = 0;
      //Is it Monday (new week came)?
      if (new Date().getDay() === 1)
      {
	      domain_data.prevweek = domain_data.thisweek;
	      domain_data.thisweek = 0;      	
      }
        ...
        if (state === "active") {
          chrome.tabs.getSelected(null, function (tab) {
            ...
            if (!inBlacklist(tab.url)) {
              ...
              domain_data.thisweek += UPDATE_INTERVAL;
              domain_data.all += UPDATE_INTERVAL;
              localStorage[domain] = JSON.stringify(domain_data);
              // Update total time
              var total = JSON.parse(localStorage["total"]);
              total.today += UPDATE_INTERVAL;
              total.thisweek += UPDATE_INTERVAL;
